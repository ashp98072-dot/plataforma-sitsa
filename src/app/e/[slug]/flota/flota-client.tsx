...
/** Cliente pesado: se carga solo en el browser (evita HTML truncado en Hostinger). */
export default function FlotaClient() {
  ...
  const can = useCallback(
    (sub: string, accion: "ver" | "crear" | "editar" | "eliminar" = "ver") => {
      if (!sesionLista) return false;
      if (isAdmin) return true;
      return tienePermiso(permisos, sub, accion);
    },
    [isAdmin, permisos, sesionLista],
  );
  ...
  const cargar = useCallback(
    async (opts?: { forzarTodo?: boolean; solo?: string[] }) => {
      ...
      try {
        ...
        if (forzar) {
          for (const k of Object.keys(cacheAt.current)) {
            cacheAt.current[k] = 0;
          }
        }
  ...
  function irARegistrarSalidaDeTaller(v: Vehiculo) {
    setVehiculoId(v.id);
    setKmLectura(Number(v.km_actual ?? 0));
    setSacarDeServicio(true);
    setFechaSalidaTaller(hoyLocal());
    if (v.fecha_entrada_taller) {
      setFechaEntradaTaller(String(v.fecha_entrada_taller).slice(0, 10));
    }
    ...
    setTab("servicios");
    setMsg(
      `Completa y guarda el servicio de ${v.placa} (marcará salida de taller).`,
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {tab === "taller" && can("flota_servicios") ? (
        <div className="space-y-4">
          {SearchBar}
          <section className="space-y-3 rounded-xl border border-[var(--border)] bg-[var(--card)] p-4">
            <div>
              <h2 className="text-sm font-semibold">
                Vehículos en servicio / taller
              </h2>
              <p className="text-xs text-[var(--muted)]">
                Unidades en mantenimiento o reparación.
                {resumen ? (
                  <>
                    {" "}
                    · En taller: <strong>{resumen.enTaller}</strong>
                ...
                (marcar salida de taller al guardar).
              </p>
            </div>
                        ...
                        >
                          ...
                          <td className="px-3 py-2">
                            <button
                              ...
                              onClick={() => irARegistrarSalidaDeTaller(v)}
                            >
                              Sacar vía registrar servicio
                            </button>
                          </td>
                        </tr>
                      );
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  {editServicioId
                    ...
                    : "No se puede registrar servicio si la unidad está en ruta. Para sacar de taller, marca la casilla al guardar."}
                  </p>
                </div>
                {editServicioId ? (
                  <button
      ...
      {tab === "historial-servicios" && can("flota_servicios") ? (
        <div className="space-y-4">
          {SearchBar}
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-end gap-3 rounded-xl border border-[var(--border)] bg-[var(--card)] p-4">
            ...
            <button
              ...
              onClick={() => void cargar({ forzarTodo: true })}
            >
              Actualizar gráficas
            </button>
            <p className="text-sm text-[var(--muted)]">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-end">
                  <button
                    ...
                    disabled={subiendoFotos || enviandoForm}
                    onClick={() => void salidaViaje()}
                  >
                    {subiendoFotos || enviandoForm
                      ? "Guardando…"
                      : "Guardar salida"}
                  </button>
                </div>
