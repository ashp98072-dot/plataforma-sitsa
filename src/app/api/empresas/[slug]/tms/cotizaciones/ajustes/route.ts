...
export async function GET(_req:Request,ctx:Ctx){
  ...
  try { const hoy=hoyGuatemala(); const [parametros,perfiles]=await Promise.all([listarParametrosAjustes(guard.empresa.id,hoy),listarPerfilesAjustes(guard.empresa.id)]);
    return NextResponse.json({hoy,parametros,perfiles},{headers:H});
