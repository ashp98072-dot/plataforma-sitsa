...
export async function GET(_req: Request, ctx: Ctx) {
  ...
  return NextResponse.json({ cxc: rows });
}
...
export async function POST(req: Request, ctx: Ctx) {
  ...
  const result = await execute(
    `INSERT INTO cont_cxc (empresa_id, cliente, documento, fecha, vencimiento, monto, saldo, estado)
     VALUES (?, ?, ?, ?, ?, ?, ?, 'Pendiente')`,
    [
      guard.empresa.id,
      d.cliente,
  ...
  return NextResponse.json({ id: result.insertId, mensaje: "CxC registrada." });
}
