...
const req = (body = "{}") => new Request("http://localhost/api/prueba", { method: "POST", body });
beforeEach(() => {
  vi.resetAllMocks();
  vi.mocked(requireTenantModulo).mockResolvedValue({ empresa: { id: 7 }, session: { username: "real" } } as never);
...
it("empresa y usuario provienen del guard, conserva respuesta compatible", async () => {
  ...
  expect(registrarAsiento).toHaveBeenCalledWith(7, "real", expect.anything());
...
it("JSON inválido llega como null al validador y responde 400", async () => {
  ...
  expect(registrarAsiento).toHaveBeenCalledWith(7, "real", null);
});
