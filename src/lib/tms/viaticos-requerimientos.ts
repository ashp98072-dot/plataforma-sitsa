...
function mapCabecera(r: RowDataPacket): RequerimientoViatico {
  return { ...r, id: Number(r.id), codigo: String(r.codigo), estado: r.estado, total: String(r.total), fecha_requerimiento: fechaSql(r.fecha_requerimiento) } as RequerimientoViatico;
}
...
async function snapshotLinea(conn: PoolConnection, empresaId: number, l: GuardarRequerimientoViatico["lineas"][number]) {
  const [p] = await conn.query<RowDataPacket[]>(`SELECT tp.id,tp.id_empleado,tp.nombre,COALESCE(e.categoria_ops,tp.tipo) cargo,
