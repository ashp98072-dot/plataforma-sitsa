...
const detalle = { id: 12, codigo: "RC-2026-000012", fecha_requerimiento: "2026-09-17", estado: "Autorizada", version: 3, entidad_requirente_nombre: "Empresa real", requirente_nombre: "Persona real", solicitante_nombre: "Autor original", total: "10.25", lineas: [{ id: 21, fecha: "2026-09-17", unidad_descripcion: "C-123ABC · Cabezal", vehiculo_id: 5, proveedor_id: 3, proveedor_nombre_snapshot: "Proveedor histórico", repuesto_descripcion: "Filtro", total: "10.25", metodo_pago: "Efectivo", condicion_pago: "Contado" }] } as DetalleCompra;
it("detalle solo lectura usa snapshots históricos y conserva total sin acciones futuras", () => {
  ...
  for (const texto of ["Empresa real", "Persona real", "Autor original", "Proveedor histórico", "C-123ABC", "10.25"]) expect(html).toContain(texto);
...
it("alta tiene todos los campos, múltiples líneas, tarjetas responsive y permisos de listado", () => {
  ...
  for (const texto of ["Empresa requirente", "Persona que requiere", "Usuario actual", "Serie factura", "Número factura", "Repuesto a comprar", "Método de pago", "Condición de pago", "Contado", "Crédito", "Agregar línea", "md:grid-cols-2"]) expect(html).toContain(texto);
...
it("no SQL, RRHH, Fondos/Gastos, uploads ni credenciales en el diff de Fase 2", () => {
  const files = execFileSync("git", ["diff", "--name-only", "dded5ef1398f7808d3bb5c9921d48923886aa22d"], { encoding: "utf8" }).trim().split(/\r?\n/);
  expect(files.some(p => /^(sql\/)|rrhh|planillas|portales-proveedores|src\/lib\/tms\/|\/(fondos|gastos|programacion)\//i.test(p))).toBe(false);
