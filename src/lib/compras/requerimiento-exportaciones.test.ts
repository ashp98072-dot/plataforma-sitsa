...
import { COLUMNAS_EXCEL_COMPRA, requerimientoCompraExcel, requerimientoCompraPdf } from "./requerimiento-exportaciones";
...
it.each(["Pendiente", "Rechazada", "Autorizada"] as const)("PDF %s válido, completo y con firma solo si Autorizada", async estado => {
  ...
  if (estado === "Autorizada") {
    ...
    for (const dato of ["Gerente Histórico", firma.rol, "18/09/2026 10:15:00", firma.codigo]) expect(contenido).toContain(dato);
  } else {
