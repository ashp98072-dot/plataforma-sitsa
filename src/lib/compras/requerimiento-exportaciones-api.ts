...
import { firmasHistoricasCompraReporte } from "./requerimiento-firma-reporte";
...
export async function requerimientoExportar(slug: string, id: string, formato: "pdf" | "excel") {
  ...
  try {
    ...
    const firmas = formato === "pdf" ? await firmasHistoricasCompraReporte(guard.empresa.id, d.id, d.estado === "Autorizada") : null;
    // No emitir constancia autorizada sin firma histórica ni sustituirla por Mi firma.
    if (formato === "pdf" && d.estado === "Autorizada" && !firmas?.autorizante?.imagen) return respuesta("No está disponible la firma histórica de autorización.", 409);
    const buffer = formato === "pdf" ? await requerimientoCompraPdf(d, guard.empresa.nombre, firmas) : await requerimientoCompraExcel(d, guard.empresa.nombre);
