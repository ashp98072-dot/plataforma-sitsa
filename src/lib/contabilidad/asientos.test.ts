...
vi.mock("./ambito", () => ({ bloquearAmbito: vi.fn(), exigirEsquemaC2b: vi.fn() }));
const ambito = { entidadId: 9, usuarioId: 4, admin: false };
...
beforeEach(() => {
  ...
  conn.query.mockResolvedValue([[{ id: 1, empresa_id: 7, entidad_id: 9, activa: 1 }, { id: 2, empresa_id: 7, entidad_id: 9, activa: 1 }]]);
...
it("registra cabecera, detalle y auditoría en la misma conexión, con importes exactos", async () => {
  expect(await registrarAsiento(7, "prueba", datos(), ambito)).toBe(10);
  expect(conn.beginTransaction).toHaveBeenCalledOnce();
  expect(conn.query).toHaveBeenCalledWith(expect.stringMatching(/empresa_id = \?[\s\S]*ORDER BY id FOR UPDATE/), [7, 9, 1, 2]);
  expect(conn.execute.mock.calls[1][1]).toEqual([7, 9, 10, 2, "0.30", "0.00"]);
...
it.each([
  ...
  await expect(registrarAsiento(7, "prueba", { ...datos(), ...cambio }, ambito)).rejects.toThrow("Datos inválidos");
  expect(getPool).not.toHaveBeenCalled();
});
it("rechaza descuadre de un centavo sin escribir", async () => {
  const d = datos(); d.lineas[0].debe = 0.31;
  await expect(registrarAsiento(7, "prueba", d, ambito)).rejects.toThrow("no cuadra");
  expect(getPool).not.toHaveBeenCalled();
});
it.each([
  [], [{ id: 1, empresa_id: 7, entidad_id: 9, activa: 1 }],
  [{ id: 1, empresa_id: 8, entidad_id: 9, activa: 1 }, { id: 2, empresa_id: 7, entidad_id: 9, activa: 1 }],
  [{ id: 1, empresa_id: 7, entidad_id: 9, activa: 0 }, { id: 2, empresa_id: 7, entidad_id: 9, activa: 1 }],
  ...
  await expect(registrarAsiento(7, "prueba", datos(), ambito)).rejects.toThrow("Todas las cuentas");
...
it.each(["cabecera", "detalle", "auditoria"])("rollback si falla %s", async (paso) => {
  ...
  await expect(registrarAsiento(7, "prueba", datos(), ambito)).rejects.toThrow("fallo");
  expect(conn.rollback).toHaveBeenCalledOnce();
  expect(conn.commit).not.toHaveBeenCalled();
  expect(conn.release).toHaveBeenCalledOnce();
});
it("preserva el error de número duplicado y revierte", async () => {
  ...
  await expect(registrarAsiento(7, "prueba", datos(), ambito)).rejects.toMatchObject({ code: "ER_DUP_ENTRY" });
  expect(conn.rollback).toHaveBeenCalledOnce();
});
it("acepta totales mayores al entero seguro sin perder centavos", async () => {
  ...
  await registrarAsiento(7, "prueba", { ...datos(), lineas }, ambito);
  expect(conn.execute.mock.calls[1][1]).toEqual([7, 9, 10, 1, "999999999999.99", "0.00"]);
...
it("si falla el bloqueo no escribe y libera la conexión", async () => {
  ...
  await expect(registrarAsiento(7, "prueba", datos(), ambito)).rejects.toMatchObject({ code: "ER_LOCK_DEADLOCK" });
  expect(conn.execute).not.toHaveBeenCalled();
  expect(conn.rollback).toHaveBeenCalledOnce();
  expect(conn.release).toHaveBeenCalledOnce();
});
