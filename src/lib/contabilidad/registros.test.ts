...
vi.mock("./ambito", () => ({ bloquearAmbito: vi.fn(), exigirEsquemaC2b: vi.fn(), errorAmbito: vi.fn() }));
...
it.each(["cuentas", "cxc", "cxp"] as TipoRegistro[])("registra %s con tenant y auditoría en la misma transacción", async (tipo) => {
  expect(await crearRegistro(tipo, 7, "actor", { ...(tipo === "cuentas" ? cuenta : obligacion), empresaId: 99, usuario: "falso", saldo: 99 }, ambito)).toBe(11);
  expect(conn.execute.mock.calls[0][1][0]).toBe(7);
  if (tipo === "cuentas") expect(conn.execute.mock.calls[0][1]).toEqual([7, 9, "001", "Caja", "Activo", 1]);
...
it.each([{ codigo: " " }, { nombre: "x".repeat(201) }, { codigo: "x".repeat(41) }, { nivel: 0 }, { nivel: 1.5 }, { tipo: "Otro" }])("valida cuenta %#", async (d) => {
  await expect(crearRegistro("cuentas", 7, "actor", { ...cuenta, ...d }, ambito)).rejects.toThrow();
...
it.each(["cxc", "cxp"] as const)("valida fechas, límites e importes de %s", async (tipo) => {
  ...
  ]) await expect(crearRegistro(tipo, 7, "actor", { ...obligacion, ...d }, ambito)).rejects.toThrow();
  expect(getPool).not.toHaveBeenCalled();
});
it("admite fecha bisiesta, cero legado y vencimiento vacío como NULL", async () => {
  await crearRegistro("cxc", 7, "actor", { ...obligacion, fecha: "2024-02-29", monto: 0, vencimiento: "" }, ambito);
  expect(conn.execute.mock.calls[0][1]).toEqual([7, 9, "Cliente ficticio", null, "2024-02-29", null, "0.00", "0.00"]);
});
it.each(["cuentas", "cxc", "cxp"] as TipoRegistro[])("rollback completo de %s ante fallo de insert o auditoría", async (tipo) => {
  ...
  await expect(crearRegistro(tipo, 7, "actor", body, ambito)).rejects.toThrow("insert");
  vi.mocked(registrarAuditoriaTx).mockRejectedValueOnce(new Error("audit"));
  await expect(crearRegistro(tipo, 7, "actor", body, ambito)).rejects.toThrow("audit");
