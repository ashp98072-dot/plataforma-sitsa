...
La migración sobre el estado real es **NO-OP**: no se necesita DDL para capturar antecedentes. No hay script mutativo nuevo ni CREATE/ALTER/DROP de producción. No ejecutar `schema.sql` contra producción. El archivo `preflight-2026-09-rrhh-fiscal-modelo.sql` es una propuesta reejecutable de solo lectura para contrastar VERSION, engines, tipos exactos, índices, FKs y huérfanos; no fue ejecutado por este PR.
