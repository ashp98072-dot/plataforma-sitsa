...
PriceSmart, según lo que ya se ve en datos reales del sistema (plan
`PLAN-20260901-005`, cliente `PRICESMART`), **ya es un cliente real de
SITSA que hoy usa el Portal del Cliente de forma manual** — una persona
de PriceSmart entra, llena el formulario de "Nueva solicitud" y un
operador de SITSA la revisa y programa.
...
2. No hay `external_order_id`, ni tabla ni mecanismo para prevenir
   pedidos duplicados de una fuente que puede reintentar.
...
4. No existe ningún motor de optimización de rutas en el repo — sería
   ...
   `programarSolicitud()`, nunca reemplazándolo.
5. Sí existe ya, parcialmente, el concepto de "hora planificada vs hora
   real" (`regreso_estimado` vs `flota_viajes.hora_llegada`), que es la
   base directa para un KPI de puntualidad (OTD).
6. No existe ningún mecanismo de autenticación máquina-a-máquina en todo
   el repo (0 ocurrencias de "api key"/"webhook" en `src/`) — el
   adaptador necesitaría un guard nuevo, del mismo estilo que
   `requireTenant*`/`requireClienteSession()` pero pensado para un
   sistema, no para una persona.
...
3. **Actor no-humano** — `creado_por_usuario_cliente_id` es `NOT NULL`
   ...
   de Portal detrás. Ver diseño propuesto en la sección de esquema
   hipotético (§14) — la opción recomendada es un
   `creado_por_integracion_id` **aparte**, no forzar un usuario-cliente
   sintético en la tabla de humanos.
...
su formato), en una columna nueva — nunca reutilizando ninguna columna
existente (`referencia_cliente` ya se usa para otra cosa: una referencia
libre que el cliente escribe a mano en el Portal, y mezclar ambos
conceptos rompería el Portal actual).

Propuesta conceptual (sin crear nada todavía, ver §14):

```
tms_solicitudes_cliente.external_order_id  VARCHAR(120) NULL
tms_solicitudes_cliente.origen_integracion VARCHAR(40)  NULL  -- 'PRICESMART', futuro-proof para otros clientes con API
UNIQUE KEY (empresa_id, cliente_id, origen_integracion, external_order_id)
```

El `UNIQUE` compuesto es lo que resuelve el punto 4 (duplicados) — nunca
un `if` aplicativo solo.
...
vez de un 500 genérico. Aplicado aquí: el `UNIQUE (empresa_id,
cliente_id, origen_integracion, external_order_id)` propuesto arriba es
la defensa real contra un reintento de PriceSmart (timeout de su lado,
reenvío del mismo pedido, etc.) — el adaptador captura ese error
específico y responde "ya lo tenemos" en vez de crear una segunda
solicitud.
...
recepción del pedido validado (payload de PriceSmart → filas candidatas
de `tms_solicitud_paradas`) **y** `programarSolicitud()` /la conversión a
plan. Es decir, el optimizador operaría sobre datos que ya tienen la
forma de "solicitud con paradas" — nunca directamente sobre el JSON
crudo de PriceSmart, y nunca escribiendo directo a `tms_planes_viaje` sin
pasar por la misma conversión transaccional que ya usa Operaciones. Esto
mantiene una sola vía de creación de planes, consistente con "no crear
una segunda fuente de verdad" (mismo principio ya aplicado en
CLIENTE-PORTAL-4 para evidencias/seguimiento).

Si el optimizador propone **agrupar varios pedidos de PriceSmart en un
mismo viaje**, eso es una decisión de negocio mayor (¿una solicitud por
pedido, o una solicitud que representa varios pedidos agrupados?) que
requiere confirmación explícita — no se asume ninguna de las dos aquí
(ver §16, decisiones pendientes).
...
Ya existe, parcialmente, sin construir nada nuevo:

| Concepto | Planificado | Real |
|---|---|---|
| Salida/carga | `tms_planes_viaje.hora_carga` | `flota_viajes.hora_salida` |
| Llegada/entrega | `tms_planes_viaje.regreso_estimado` | `flota_viajes.hora_llegada` |

Lo que falta es una vista/consulta que las **cruce** explícitamente por
`plan_id` (vía `flota_viajes.plan_id`, ya existe como columna nullable —
`src/lib/flota/schema.ts:426-427`) y calcule la diferencia — hoy cada
mitad vive en su propia tabla y ningún reporte las junta más allá del
indicador `atrasado` (en vivo, no histórico — §1.7).

## 10. Cómo calcular el KPI OTD (punto 9)

Con los datos de la sección anterior, el cálculo mínimo viable —
**conceptual, no implementado**:

```
a_tiempo := flota_viajes.hora_llegada <= tms_planes_viaje.regreso_estimado
```

sobre viajes ya cerrados (`estado IN ('Descargado','Cerrado')`) y con
`flota_viajes.plan_id` resuelto. Puntos que requieren decisión de
negocio, no supuestos técnicos:

- **Tolerancia**: ¿"a tiempo" es estricto, o se permite un margen (p. ej.
  ±30 min)? No hay ningún precedente de tolerancia en el sistema hoy.
- **Qué hora cuenta como "entrega real"**: `hora_llegada` es el registro
  del piloto en el Portal Piloto — ¿es esa la hora que también reconoce
  PriceSmart como "entregado", o su sistema tiene su propia hora de
  recepción en bodega que habría que reconciliar por separado?
- **Denominador**: ¿el OTD se calcula sobre viajes cerrados, o también
  cuenta cancelados/rechazados como "no cumplidos"?

Ninguna de estas se puede resolver sin confirmación de negocio — se deja
explícitamente pendiente, no se inventa un criterio.
...
- Un indicador de OTD por solicitud/viaje, si el negocio confirma que
...
en este proyecto para todo lo demás, en vez de traer algo externo:
...
- **Ruta dedicada** bajo `/api/integraciones/pricesmart/pedidos` (o
  ...
  de la API (`src/app/api/...`), no un servicio aparte.
- **Guard nuevo, no reutilizar `requireTenant*`/`requireClienteSession()`**:
  esos dos asumen una sesión de un humano (cookie + JWT). Un adaptador
  necesita su propio guard tipo `requireIntegracionApiKey()` — valida un
  secreto (header, nunca en query string — mismo criterio de seguridad
  ya aplicado en todo el proyecto) contra un registro por
  empresa+integración, y resuelve `empresaId`/`clienteId` desde ESE
  registro, nunca desde el body de la petición (mismo principio que ya
  aplica `requireClienteSession()`: la identidad nunca viene del
  cliente).
...
- **Flujo interno**: validar payload → verificar `external_order_id` no
  duplicado (§5) → registrar en el log de payloads (§6) → crear
  ...
  `crearSolicitudCliente()` → responder con el id interno (y el
...
ALTER TABLE tms_solicitudes_cliente
  ADD COLUMN origen_integracion VARCHAR(40) NULL,      -- NULL = Portal (humano), 'PRICESMART' = API
  ADD COLUMN external_order_id VARCHAR(120) NULL,
  ADD COLUMN creado_por_integracion_id INT NULL,        -- alternativa a forzar un usuario-cliente sintético
  ADD UNIQUE KEY uq_tmssolicli_pedido_externo
    (empresa_id, cliente_id, origen_integracion, external_order_id);

-- OJO: creado_por_usuario_cliente_id sigue NOT NULL hoy — habría que
-- decidir si se vuelve NULLABLE (con CHECK aplicativo "uno de los dos
-- debe existir") o si se crea una fila "sistema" en
-- tms_cliente_usuarios por integración. Ninguna de las dos está
-- decidida — ver §16.
...
CREATE TABLE tms_integracion_credenciales (
  id INT AUTO_INCREMENT PRIMARY KEY,
  empresa_id INT NOT NULL,
  cliente_id INT NOT NULL,
  origen_integracion VARCHAR(40) NOT NULL,
  api_key_hash VARCHAR(255) NOT NULL,   -- nunca texto plano, mismo criterio que tms_cliente_usuarios.password_hash
  activo TINYINT(1) NOT NULL DEFAULT 1,
  creado_en DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  UNIQUE KEY uq_integcred (empresa_id, cliente_id, origen_integracion)
);
```

## 15. Riesgos

- **Confundir origen de solicitudes**: si `origen_integracion` no se
  agrega, Operaciones no puede distinguir un pedido de PriceSmart de uno
  tecleado a mano — impacta soporte/reportes desde el día uno.
...
3. ¿Tolerancia de tiempo para el KPI OTD, y qué hora cuenta como
   "entregado" (la del Portal Piloto vs. una confirmación propia de
   PriceSmart)?
...
Si se necesita avanzar en paralelo mientras se espera esa confirmación,
el ticket técnico más seguro y de menor riesgo sería:

**PRICESMART-INTEGRACION-1-EXTENSION-SOLICITUDES-ORIGEN** — agregar
`origen_integracion`/`external_order_id`/`creado_por_integracion_id` a
`tms_solicitudes_cliente` (migración idempotente, revisión SQL antes de
ejecutar, mismo patrón que CLIENTE-PORTAL-1B) + la tabla
`tms_integracion_pedidos_log`, **sin** crear todavía ningún endpoint que
las use. Esto deja la base de datos lista sin comprometerse a ningún
contrato de API específico, y es reversible/de bajo riesgo si el
contrato final con PriceSmart cambia algún detalle.
...
1. **Tablas reutilizables**: `tms_clientes`/`clientes`,
   ...
   (reutilizables con extensión aditiva). Ver §2.
2. **Conceptos que faltan**: origen del pedido, id externo, actor no
   humano, log de payload crudo, motor de optimización, vista
   planificado-vs-real agregada, autenticación M2M. Ver §3.
3. **`external_order_id`**: columna nueva opaca en
   `tms_solicitudes_cliente`, nunca reutilizar `referencia_cliente`. Ver
   §4/§14.
4. **Evitar duplicados**: `UNIQUE KEY` de MySQL + helper
   `esDuplicadoXxx()`, mismo patrón ya usado 3 veces en el repo. Ver §5.
5. **Payload original/auditoría**: tabla nueva dedicada
   (`tms_integracion_pedidos_log`, JSON), `auditoria` sigue para el
   resumen humano-legible en paralelo. Ver §6.
6. **Motor de optimización**: no existe hoy, sería 100% nuevo; entraría
   entre "pedido validado" y `programarSolicitud()`. Ver §7.
7. **Pasar ruta propuesta a Programación**: crear
   `tms_solicitudes_cliente`+paradas con el origen marcado; la pantalla
   de revisión ya existe (CLIENTE-PORTAL-3), sin cambios de UI. Ver §8.
8. **Hora planificada vs real**: ya existe el dato crudo
   (`regreso_estimado`/`hora_carga` vs `hora_llegada`/`hora_salida`),
   falta solo la vista que los cruce. Ver §9.
9. **KPI OTD**: cálculo conceptual simple sobre esos mismos campos;
   tolerancia y definición de "entregado" son decisión de negocio
   pendiente. Ver §10/§16.
10. **Exponer en Portal Cliente**: ya tiene el lugar natural
    (`cliente-portal-seguimiento.ts`/detalle de solicitud), sin pantalla
    nueva; solo campos adicionales si se decide mostrarlos. Ver §11.
...
12. **Arquitectura del adaptador**: Route Handler bajo `/api/integraciones/`,
    guard nuevo tipo API-key (no reutilizar guards de sesión humana),
    validación `zod`, idempotencia por `UNIQUE` de DB, sin motor de
    optimización en la primera versión. Ver §13.
