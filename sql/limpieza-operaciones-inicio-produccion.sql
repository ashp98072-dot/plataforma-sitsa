...
-- NO borra archivos físicos. NO ejecutar si el PREVIEW informa bloqueos de facturación.
...
FROM fact_factura_viajes ffv JOIN tms_planes_viaje p ON p.id=ffv.plan_id
...
SELECT IF(@empresa_confirmada=1 AND @bloqueos_facturacion=0,
  ...
  'DETENER: empresa inválida o existen viajes facturados') AS control_previo;
