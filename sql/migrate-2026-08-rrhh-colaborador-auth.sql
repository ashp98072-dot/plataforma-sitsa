...
-- Tabla de credenciales, SEPARADA de `usuarios` (staff) y ligada 1:1 a empleados.id.
-- Un empleado puede no tener fila aquí (todavía no se le dio acceso al portal).
...
-- FK hacia empleados.id. ON DELETE CASCADE: si algún día se borra físicamente
-- un empleado (lo normal es solo desactivarlo con estado='Inactivo'), su
-- credencial de acceso no debe quedar huérfana ni permitir login fantasma.
SET @fk_exists := (
...
-- Índice para búsquedas de login por username (además del UNIQUE que ya indexa).
SET @idx_exists := (
